...
module.exports = {
	...
	// ### `public`
	//
	// When set to `true`, Nebula starts in public mode. When set to `false`,
	...
	// ### `reverseProxy`
	//
	// When set to `true`, Nebula is marked as served behind a reverse proxy
	...
	// ### `https`
	//
	// These settings are used to run Nebula's web server using encrypted TLS.
	//
	// If you want more control over the webserver,
	// [use a reverse proxy instead](https://nebula.chat/docs/guides/reverse-proxies).
	//
	...
	//
	// Nebula ships with two themes (`default` and `morning`) and can be
	// extended by installing more themes. Read more about how to manage them
	// [here](https://nebula.chat/docs/guides/theme-creation).
	//
	...
	// Solarized would be `nebula-theme-solarized`.
	//
	...
	// ### `prefetch`
	//
	// When set to `true`, Nebula will load thumbnails and site descriptions
	...
	// ### `disableMediaPreview`
	//
	// When set to `true`, Nebula will not preview media (images, video and
	...
	// images proxied via the Nebula will be previewed.
	//
	...
	// When set to `true`, Nebula will store and proxy prefetched images and
	...
	// or audio as Nebula will only load these from https hosts.
	//
	// If storage is enabled, Nebula will fetch and store images and thumbnails
	// in the `${NEBULA_HOME}/storage` folder.
	//
	// Images are deleted when they are no longer referenced by any message
	// (controlled by `maxHistory`), and the folder is cleaned up when Nebula
	// restarts.
	//
	// This value is set to `false` by default.
	...
	// before Nebula gives up attempting to fetch a link. This can be useful
	// if you've increased the `prefetchMaxImageSize`.
	//
	// Take caution, however, that an inordinately large value may lead to
	// performance issues or even a denial of service, since Nebula will not
	...
	// ### `fileUpload`
	//
	// Allow uploading files to the server hosting Nebula.
	//
	// Files are stored in the `${NEBULA_HOME}/uploads` folder, do not expire,
	// and are not removed by Nebula. This may cause issues depending on your
	...
	//   to correctly proxy the uploads URLs back to Nebula.
	...
	// one.
	//
	// This value is set to `"Nebula - https://nebula.chat"` by
	// default.
	leaveMessage: "Nebula - https://nebula.chat",
	...
	//
	// - `name`: Name to display in the channel list of Nebula. This value is
	...
	//
	// This value is set to connect to the official channel of Nebula on
	...
	// Nebula can log user messages, for example to access them later or to
	...
	// ## WEBIRC support
	//
	// When enabled, Nebula will pass the connecting user's host and IP to the
	// IRC server. Note that this requires to obtain a password from the IRC
	// network that Nebula will be connecting to and generally involves a lot
	...
	// ### `identd`
	//
	// Run Nebula with `identd` support.
	//
	...
	//
	// 1. Nebula connects to the LDAP server with its system credentials.
	// 2. It performs an LDAP search query to find the full DN associated to the
	//    user requesting to log in.
	// 3. Nebula tries to connect a second time, but this time using the
	...
	ldap: {
		...
		//   which Nebula first looks for the user DN before authenticating them.
		...
		searchDN: {
			...
			//     It is set to `"cn=nebula,ou=system-users,dc=example,dc=com"` by
			//     default.
			rootDN: "cn=nebula,ou=system-users,dc=example,dc=com",

			//   - `rootPassword`: Password of Nebula LDAP system user.
			rootPassword: "1234",
	...
	debug: {
		...
		// underlying IRC library for Node.js used by Nebula.
		ircFramework: false,
